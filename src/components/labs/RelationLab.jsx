  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-cosmic-900/90 p-5 rounded-3xl border border-cosmic-750">
        <div>
          <label className="block text-xs sm:text-sm font-bold uppercase tracking-wider text-neon-purple mb-1">
          ...
          <input
            ...
            className="w-full bg-cosmic-950 border border-cosmic-750 rounded-xl px-3.5 py-2 text-sm text-neon-purple font-mono focus:outline-none focus:border-neon-purple"
            placeholder="1, 2, 3, 4"
          />
        </div>
        <div>
          <label className="block text-xs sm:text-sm font-bold uppercase tracking-wider text-neon-pink mb-1">
          ...
          <input
            ...
            className="w-full bg-cosmic-950 border border-cosmic-750 rounded-xl px-3.5 py-2 text-sm text-neon-pink font-mono focus:outline-none focus:border-neon-pink"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-cosmic-900/90 p-5 rounded-3xl border border-cosmic-750 space-y-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-slate-400">Click cells to toggle</span>
          </div>
          ...
          <div className="overflow-x-auto p-2 bg-cosmic-950/80 rounded-2xl border border-cosmic-750/70">
            <table className="border-collapse mx-auto">
              <thead>
                <tr>
                  <th className="p-2 text-xs sm:text-sm text-slate-500 font-mono"></th>
                  {elements.map((el) => (
                    <th key={el} className="p-2 text-xs sm:text-sm text-neon-purple font-mono text-center font-bold">
                      {el}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {elements.map((u, i) => (
                  <tr key={u}>
                    <td className="p-2 text-xs sm:text-sm text-neon-purple font-mono font-bold text-right">{u}</td>
                    {elements.map((v, j) => {
                      ...
                      return (
                        <td key={v} className="p-1">
                          <button
                            ...
                            className={`btn-arcade w-9 h-9 sm:w-10 sm:h-10 rounded-xl text-xs sm:text-sm font-mono font-bold transition-all ${
                              hasPair
        ...
        <div className="space-y-4">
          <div className="bg-cosmic-900/90 p-5 rounded-3xl border border-cosmic-750 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="btn-arcade px-3.5 py-2 rounded-full text-xs sm:text-sm font-semibold bg-cosmic-950 border border-cosmic-750 text-slate-200 hover:border-neon-purple hover:text-neon-purple transition"
              >
                + Reflexive Closure <MathView math="R \cup \Delta" />
              </button>
              <button
                onClick={() => applyClosure('symmetric')}
                className="btn-arcade px-3.5 py-2 rounded-full text-xs sm:text-sm font-semibold bg-cosmic-950 border border-cosmic-750 text-slate-200 hover:border-neon-pink hover:text-neon-pink transition"
              >
                + Symmetric Closure <MathView math="R \cup R^{-1}" />
              </button>
              <button
                onClick={() => applyClosure('transitive')}
                className="btn-arcade px-3.5 py-2 rounded-full text-xs sm:text-sm font-semibold bg-cosmic-950 border border-cosmic-750 text-slate-200 hover:border-neon-cyan hover:text-neon-cyan transition"
              >
          ...
          <div className="bg-cosmic-900/90 p-5 rounded-3xl border border-cosmic-750 space-y-2">
            <h4 className="text-sm font-bold text-white mb-2 flex items-center gap-2">
              <span>Quotient Set & Partitions</span>
              <MathView math="A / R" />
            </h4>
            {analysis.isEquivalence ? (
              <div className="space-y-2">
                <p className="text-xs sm:text-sm text-neon-mint font-semibold flex items-center gap-1.5">
                  <span>✓ Valid Equivalence Relation! Generates partition of</span>
                  <MathView math="A" />:
                </p>
                <div className="flex flex-wrap gap-2">
                  {analysis.equivalenceClasses.map((cls, idx) => (
                    <div
                      key={idx}
                      className="px-3.5 py-2 bg-neon-purple/15 border border-neon-purple/40 rounded-2xl text-xs sm:text-sm font-mono font-bold text-neon-purple"
                    >
                      <MathView math={`[${cls[0]}] = \\{${cls.join(', ')}\\}`} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                <MathView text="$R$ is not yet an equivalence relation. Use the 1-click closure buttons above or toggle matrix cells to complete missing pairs." />
              </p>
            )}
          </div>
        </div>
...
function PropertyBadge({ label, holds, tooltip, highlight = false }) {
  return (
    ...
    >
      ...
      <span className="text-xs font-mono mt-0.5 opacity-80">{holds ? 'Holds' : 'Fails'}</span>
    </div>
  );
}

...
export default function CheatSheetScreen({ topic }) {
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-5xl mx-auto">
      {/* Header Banner */}
      <div className="flex flex-wrap items-center justify-between gap-4 p-5 sm:p-7 rounded-3xl bg-gradient-to-br from-cosmic-900 via-cosmic-850 to-cosmic-900 border border-neon-gold/30 shadow-xl shadow-black/40">
        <div>
        ...
        <button
          ...
          className="btn-arcade px-4 py-2.5 rounded-full text-xs sm:text-sm font-bold bg-cosmic-950/80 border border-cosmic-750 text-slate-200 hover:text-white hover:border-neon-gold/50 flex items-center gap-2"
        >
          {copied ? (
            <>
          ...
          {cheatSheet.traps.map((trap, idx) => (
            ...
            >
              <div className="flex items-center gap-2.5">
                <span className="w-7 h-7 rounded-full bg-neon-pink/20 border border-neon-pink/40 text-neon-pink text-xs font-mono font-bold flex items-center justify-center">
                  !
                </span>

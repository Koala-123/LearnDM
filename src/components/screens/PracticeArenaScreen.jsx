...
export default function PracticeArenaScreen({
  ...
  return (
    <div className="space-y-6 animate-fadeIn max-w-5xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-5 rounded-3xl bg-gradient-to-br from-cosmic-900 via-cosmic-850 to-cosmic-900 border border-neon-purple/30 shadow-xl shadow-black/40">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-0.5">
            Test yourself against Grimaldi, O'Regan, and Plaksha University midsem/endsem exam questions.
          </p>
        </div>

        {/* Stats Multiplier Chips */}
        <div className="flex items-center gap-3">
          <div className="px-3.5 py-2 rounded-2xl bg-cosmic-950/80 border border-cosmic-750 text-center">
            <div className="text-[10px] font-mono text-slate-400 uppercase">Solved</div>
            <div className="text-sm font-extrabold text-neon-mint">
              {solvedCount} / {questions.length}
            </div>
          </div>
          <div className="px-3.5 py-2 rounded-2xl bg-cosmic-950/80 border border-cosmic-750 text-center">
            <div className="text-[10px] font-mono text-slate-400 uppercase">XP Yield</div>
            <div className="text-sm font-extrabold text-neon-gold">
              +{solvedCount * 50} XP
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-2 bg-cosmic-900/80 rounded-2xl border border-cosmic-750">
            ...
            <button
              ...
              className={`btn-arcade px-3 py-1.5 text-xs rounded-full border transition ${
                selectedTier === tier.id
                  ...
                  : 'bg-cosmic-950/60 border-cosmic-750 text-slate-400 hover:text-slate-200'
              }`}
            >
              {tier.label}
            </button>
      ...
      {filteredQuestions.length === 0 ? (
        <div className="p-12 text-center text-slate-400 text-sm bg-cosmic-900/40 rounded-2xl border border-cosmic-750">
        ...
        <div className="space-y-6">
          {currentQuestions.map((q, idx) => {
            ...
            return (
              <div
                key={q.id}
                className="bg-cosmic-900/90 rounded-3xl border border-cosmic-750 p-6 space-y-4 shadow-xl shadow-black/30 hover:border-cosmic-700 transition"
              >
                {/* Header */}
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    <span className="w-7 h-7 rounded-full bg-cosmic-950 border border-cosmic-750 flex items-center justify-center text-xs font-mono font-bold text-slate-300">
                      {globalIndex}
                    </span>
                    <span
                  ...
                  {q.loadLabPayload && (
                    <button
                      ...
                      className="btn-arcade px-3 py-1.5 rounded-full text-xs font-semibold bg-neon-cyan/15 border border-neon-cyan/40 text-neon-cyan hover:bg-neon-cyan/25 flex items-center gap-1.5"
                ...
                {/* Prompt */}
                <div className="text-sm text-slate-100 font-medium leading-relaxed bg-cosmic-950/60 p-4 rounded-2xl border border-cosmic-750/70">
                  ...
                  {q.options.map((opt, optIdx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`btn-arcade p-3.5 rounded-2xl border text-left text-xs transition flex items-center justify-between ${btnClass}`}
                      >
                        <div className="flex items-center gap-3">
                          <span className="w-6 h-6 rounded-full bg-cosmic-900 border border-cosmic-750 flex items-center justify-center text-[10px] font-mono text-slate-400 shrink-0">
                        ...
                        {isAnswered && optIdx === q.correctIndex && (
                          <CheckCircle2 className="w-4 h-4 text-neon-mint shrink-0 ml-2" />
                        )}
                        {isAnswered && optIdx === userAnswer && optIdx !== q.correctIndex && (
                          <XCircle className="w-4 h-4 text-neon-pink shrink-0 ml-2" />
                        )}
                      </button>
                    );
                  })}
                ...
                {isAnswered && showExplanation[q.id] && (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold uppercase tracking-wider text-neon-gold flex items-center gap-1.5">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-4">
          <button
            ...
            className="btn-arcade p-2 rounded-xl bg-cosmic-900 border border-cosmic-750 text-slate-300 disabled:opacity-40 disabled:pointer-events-none hover:border-cosmic-600"
          >
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
            <button
              ...
              className={`btn-arcade w-9 h-9 rounded-xl border text-xs font-mono font-bold transition ${
                currentPage === pageNum
